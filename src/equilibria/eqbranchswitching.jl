function EquilibriaContinuation(
    ...
    bc!,
    g,
    x_ini::HamiltonianBranchPoint{U},
    params,
    Δs_ini::U,
    maxsteps::T;
    atol::U=eps(),
    ...
    if x_ini.tol > (atol + rtol * norm(x1, Inf))
        @warn("Branch Point tolerance is higher than default")
    end

    NS = nullspace(x_ini.jacobian)
    ...
    elseif size(NS, 2) == 2
        ...
        error("Nullity > 2: highly degenerate branch point")

    end

    push!(bp, HamiltonianBranchPoint(x_ini.x,
        x_ini.E,
        x_ini.jacobian,
        x_ini.nullvec,
        copy(Φ0),
