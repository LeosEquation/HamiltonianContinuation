function BPFinding!(f!, bc!, g, x::Array{U,1}, params, v::Array{U,1},
    ...
    if abs(det(J)) < (atol + rtol * norm(q, Inf))
        @warn("System Jacobian was been singular in BP bifurcation. Stopping continuation.")
        for qq in Δq
            qq = NaN
        end
        return nothing
    ...
    while k <= ite && norm(Δq, Inf) > (atol + rtol * norm(q, Inf))
...
#     while k <= ite && norm(Δq) > ( atol + rtol * norm(q1, Inf) )
