function OrbitContinuation(
    ...
    atol::U=eps(),
    rtol::U=eps(),
    ite::T=20,) where {U<:Real,T<:Integer}

    println("Running Orbit Continuation.")

    ###

    m_ini, n = size(x_ini.x)
    nξ = n - 2
    nλ = 2
    m = 2 * m_ini - 1
    Nξ = m * nξ
    ...
    comp3 = 0
    for comp1 in 1:m_ini
        for comp2 in 1:nξ
            comp3 += 1
            x0[comp3] = x_ini.x[comp1, comp2]
        end
    end

    for comp1 in 2:m_ini
        for comp2 in 1:nξ
            comp3 += 1
            x0[comp3] = x_ini.x[comp1, comp2]
        end
    ...
    x0[Nξ+1] = 2.0 * x_ini.x[1, nξ+1]
    x0[Nξ+2] = x_ini.x[1, nξ+2]
    ...
    comp3 = 0
    for comp1 in 1:(m_ini-1)
        for comp2 in 1:nξ
            comp3 += 1
            Φ0[comp3] = x_ini.v[comp1, comp2]
        end
    end
    ...
    for comp1 in 1:(m_ini-1)
        for comp2 in 1:nξ
            comp4 += 1
            xFP[comp4] = x_ini.x[comp1, comp2]
        end
        for comp2 in 1:nξ
            comp4 += 1
            xFP[comp4] = x_ini.v[comp1, comp2]
        end
    end
    for comp1 in 1:m_ini
        for comp2 in 1:nξ
    ...
    xFP[NξFP+1] = 2.0 * x_ini.x[1, nξ+1]
    xFP[NξFP+2] = x_ini.x[1, nξ+2]
    ...
    for comp1 in 1:nξFP
        xFP_temp[comp1] = xFP[comp1]
    end

    for comp1 in 1:nλ
        xFP_temp[nξFP+comp1] = xFP[NξFP+comp1]
    end
    ...
    Threads.@threads for comp1 in 1:m-1
        ...
        taylorinteg_wrap_optim!(f!, bc!, lims, xTview, 0.0, Δτ, integtol, caches[comp1], params; maxsteps=integmaxsteps)
    end
    ...
    E1 = H(x1[end-n+1:end], params, one(U))

    if sign(E1 - E0) != sign(Δs)
        Δs = sign(E1 - E0) * abs(Δs)
    end

    # print(" \r Progress : 1.0 %")
    ...
    while step <= maxsteps
        ...
        if sign(FPtest1) != sign(FPtest0)
            ...
            for comp1 in 1:m
                ...
                dx0_view = @view dx0FP[((comp1-1)*nξFP+1):((comp1-1)*nξFP+nFP)]
            ...
            push!(FP, HamiltonianFlipPoint(copy(τ), copy(ξFP_eval), copy(EFP), copy(vFP_eval), Complex.(pS.values), step - 1, norm(ΔxFP, Inf)))
