"""
Run a numerical continuation of periodic orbits.

This function implements a Newton-type adaptive corrector and a pseudo–arclength
continuation scheme to compute branches of periodic orbits of a Hamiltonian
dynamical system. Starting from an initial periodic orbit, it follows the branch,
detects period-doubling (flip) points, and can automatically continue the emerging
branch associated with such bifurcations.

Two main entry points are provided: one where the initial point is given as a
state vector, and one where it is given as a previously detected flip point.

# Methods Summary

    OrbitContinuation(f!, g!, H, bc!, lims, x_ini::Array{U,1}, params,
                      Δs_ini::U, maxsteps::T, Npoints::T, integtol::U,
                      integorder::T; integmaxsteps::T=500, parse_eqs::Bool=true,
                      atol::U=eps(), rtol::U=eps(), ite::T=20, aite::T=3)

    OrbitContinuation(f!, g!, H, bc!, lims, x_ini::HamiltonianFlipPoint{U},
                      params, Δs_ini::U, maxsteps::T, integtol::U,
                      integorder::T; integmaxsteps::T=500, parse_eqs::Bool=true,
                      atol::U=eps(), rtol::U=eps(), ite::T=20, aite::T=3)

# Arguments
- `f!`: In-place function for the differential equations of the dynamical system.
        For format details, see the TaylorIntegration documentation.
- `g!`: In-place function for the variational equations. Format follows the
        TaylorIntegration conventions.
- `H`: Hamiltonian function of the system.
- `bc!`: Boundary-condition function for the periodic-orbit problem.
- `lims`: Numerical limits used internally by the integrator.
- `x_ini`: Initial point of the branch. Use `Array{U,1}` for a regular periodic orbit,
           or `HamiltonianFlipPoint{U}` to start continuation from a previously
           computed flip point.
- `params`: Parameter container for the dynamical system (see TaylorIntegration).
- `Δs_ini`: Initial continuation step size. This step is kept fixed unless Newton
            correction fails, in which case it is reduced adaptively.
- `maxsteps`: Maximum number of continuation steps.
- `Npoints`: Number of time nodes used to discretize each periodic orbit.
- `integtol`: Integration tolerance passed to TaylorIntegration.
- `integorder`: Taylor integration order.
- `integmaxsteps`: Maximum number of Taylor steps during integration.
- `parse_eqs`: Enables the use of `@taylorize` within TaylorIntegration.
- `atol`: Absolute tolerance for Newton convergence.
- `rtol`: Relative tolerance for Newton convergence.
- `ite`: Maximum number of Newton iterations per continuation step.
- `aite`: Maximum number of adaptive step-size reductions on failed corrector steps.

# Returns
A `HamiltonianPeriodicBranch{U}` containing:

- `t::LinRange{U}`: Time nodes shared by all periodic orbits in the branch.
- `x::Array{U,3}`: Array `x[i, j, k]` where:
    * `i` is the continuation step,
    * `j` is the time-node index,
    * `k` is the coordinate index.
- `E::Array{U,1}`: Energy at each continuation step.
- `fp::Vector{HamiltonianFlipPoint{U}}`: Detected flip (period-doubling) points.
- `μ::Array{Complex{U},2}`: Floquet multipliers `μ[i, j]` for step `i`.
- `stbl::Array{Bool,1}`: Stability indicator (`true` = stable).
- `tol::Array{U,1}`: Infinity-norm correction error at each continuation step.
"""
function OrbitContinuation(
    ...
    for comp1 in 1:nλ
        x0[Nξ+comp1] = x_ini[nξ+comp1]
    end

    E0 = H(x_ini, params, 0.0)
    ...
    x_temp = copy(x_ini)
    cache_ini = TaylorIntegration.init_cache_optim(zero(U), x_ini, integorder, f!, params; parse_eqs)

    comp2 = 0
    for comp1 in 1:nξ
        comp2 += 1
        xFP_temp[comp2] = x_ini[comp1]
    end
    for comp1 in 1:nξ
        comp2 += 1
        xFP_temp[comp2] = 1.0
